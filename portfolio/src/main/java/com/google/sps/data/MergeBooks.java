...
public class MergeBooks {
  ...
  public static Book merge(Book original, Book addition) {
    validate(original);
    validate(addition);
    String isbn = getNonNullString(original.isbn(),
    ...
    combined_builder.publishedDate(
        getNonNullString(original.publishedDate(), addition.publishedDate()));
    combined_builder.publisher(getNonNullString(original.publisher(), addition.publisher()));
    combined_builder.maturityRating(
        getNonNullString(original.maturityRating(), addition.maturityRating()));
   ...
   */
  private static ImmutableList<String> getNonNullList(
      ImmutableList<String> original, ImmutableList<String> addition) {
    if (original == null && addition != null) {
      return addition;
    }
